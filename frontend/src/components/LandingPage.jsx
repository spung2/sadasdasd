...
function FeatureCard({ icon: Icon, title, desc, color, delay }) {
  return (
    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay }}
      className="p-6 rounded-xl bg-zinc-900/50 border border-white/5 hover:border-white/10 transition-all group">
...
function StatPill({ value, label }) {
  return (
    <div className="text-center">
      <p className="text-2xl sm:text-3xl font-heading font-bold text-white">{value}</p>
    ...
    <div className="min-h-screen bg-[#09090b] noise-bg overflow-hidden">
      ...
      <section className="relative max-w-[1200px] mx-auto px-6 pt-16 sm:pt-24 pb-20">
        ...
        <div className="relative z-10 text-center">
          ...
          <motion.h2 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-valorant to-valorant/70">Curated for You</span>
          </motion.h2>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }}
            ...
            <button data-testid="explore-marketplace-btn" onClick={() => navigate('/market')}
              className="inline-flex items-center justify-center gap-2 px-7 py-3.5 bg-valorant text-white font-heading font-bold text-sm uppercase tracking-widest rounded-lg hover:bg-valorant-hover transition-all shadow-[0_0_30px_rgba(255,70,85,0.2)] hover:shadow-[0_0_40px_rgba(255,70,85,0.35)]">
              Explore Marketplace <ArrowRight className="w-4 h-4" />
            </button>
            <button data-testid="learn-more-btn" onClick={() => document.getElementById('features')?.scrollIntoView({ behavior: 'smooth' })}
              className="inline-flex items-center justify-center gap-2 px-7 py-3.5 bg-zinc-900 border border-white/10 text-zinc-300 font-body text-sm rounded-lg hover:bg-zinc-800 hover:text-white transition-all">
              Learn More
            </button>
          </motion.div>
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.5 }}
          ...
          <StatPill value="Real" label="LZT API Data" />
      ...
      <section id="features" className="max-w-[1200px] mx-auto px-6 py-20">
        <div className="text-center mb-12">
          <h3 className="text-2xl sm:text-3xl font-heading font-bold text-white">Why LZT Vault?</h3>
          <p className="text-sm text-zinc-500 mt-2 max-w-md mx-auto">Every feature built for serious account traders.</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          <FeatureCard icon={Sparkles} title="Real Skin Gallery" desc="See actual weapon skins from the account's inventory with hi-res images from Valorant API." color="#fbbf24" delay={0.1} />
          <FeatureCard icon={Shield} title="Verified Data" desc="Every stat — rank, level, VP, Radianite — comes directly from the LZT Market API. No fakes." color="#00e5ff" delay={0.15} />
          ...
          <FeatureCard icon={TrendingUp} title="Smart Filters" desc="Rank range, min skins, knife toggle, price range, currency — find exactly what you need." color="#ff4655" delay={0.3} />
          <FeatureCard icon={Zap} title="Instant Preview" desc="Click any account for a rich detail modal with stats, skins, security status, and buy link." color="#fbbf24" delay={0.35} />
        </div>
      </section>
      ...
      <section className="max-w-[1200px] mx-auto px-6 pb-24">
        <div className="relative p-10 sm:p-14 rounded-2xl bg-zinc-900/60 border border-white/5 overflow-hidden text-center">
          ...
          <div className="relative z-10">
            ...
            <button data-testid="cta-marketplace-btn" onClick={() => navigate('/market')}
              className="inline-flex items-center gap-2 px-8 py-3.5 mt-6 bg-valorant text-white font-heading font-bold text-sm uppercase tracking-widest rounded-lg hover:bg-valorant-hover transition-all animate-neon-pulse">
      ...
      <footer className="border-t border-white/5 py-6">
        <div className="max-w-[1200px] mx-auto px-6 flex items-center justify-between">
          <p className="text-xs text-zinc-600">LZT Vault - Premium Account Marketplace</p>
          <p className="text-xs text-zinc-600">Powered by LZT Market API</p>
        </div>
      </footer>
    </div>
  );
