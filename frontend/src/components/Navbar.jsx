import { Crosshair, ShieldCheck, Flame, Store, LogOut, Heart } from 'lucide-react';
      ...
      <div className="max-w-[1600px] mx-auto px-4 md:px-8 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-1">
          {[
            ...
            <button key={item.label} data-testid={`nav-${item.label.toLowerCase().replace(' ', '-')}`} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${item.active ? 'text-white bg-white/5' : 'text-zinc-500 hover:text-zinc-300 hover:bg-white/[0.03]'}`}>
        ...
        <div className="flex items-center gap-3">
          ...
          {user ? (
            <div className="flex items-center gap-2">
              ...
              <button data-testid="logout-btn" onClick={handleLogout} className="p-2 text-zinc-500 hover:text-white transition-colors" title="Sign out">
                <LogOut className="w-4 h-4" />
              </button>
            </div>
          ) : (
            <button data-testid="sign-in-btn" onClick={handleLogin} className="px-4 py-2 text-sm font-semibold text-white bg-valorant/90 hover:bg-valorant rounded-lg transition-all hover:shadow-[0_0_20px_rgba(255,70,85,0.3)]">
              Sign In
            </button>
          )}
        </div>
